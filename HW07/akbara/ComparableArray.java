...
\   * return -1 if this < a, 0 if this equals a, 1 if this > a
   ...
   public int compareTo(ComparableArray a) {
      for(int i = 0; i < this.ary.length; i++) {
         if (i < a.ary.length)
         {
            ...
            else if (this.ary[i] < a.ary[i]) { return -1;}
         }
         else
         {
            for(int x = 0; x < this.ary.length; x++) {
               if (this.ary[x] != 0) { return 1;}
            }
            return 0;
         }
      }
      for(int z = 0; z < a.ary.length; z++) {
         if (a.ary[z] != 0) { return -1;}
      }
      return 0;  
   }
   ...
   public void makeNumber(int n) {
      for(int i = 0; i < ary.length; i++) { ary[i] = n;}
   }

   // print out the elements of ary
   public String toString( ){
      String elements = "[" + ary[0];
